...
function drawMap () {
  ...
  window.addEventListener('resize', function () {
    ...
    let textureRivers =
      PIXI.utils.TextureCache['resources/img/roads_rivers-tileset.png'];
...
function getDefaultSettings () {
  ...
  let rows =
    Math.ceil(height / (16 * 1.732_05)) > 20
      ? 20
      : Math.ceil(height / (16 * 1.732_05));
...
function initializeViewport (app, settings) {
  let worldWidth =
    ...
    settings.hexSize / 2;
  let worldHeight =
    settings.hexRows * (settings.hexSize * 1.732_05) +
    (settings.hexSize * 1.732_05) / 2;

  if (settings.hexOrientation === 'pointy') {
    worldWidth =
      settings.hexColums * (settings.hexSize * 1.732_05) +
      (settings.hexSize * 1.732_05) / 2;
    worldHeight =
...
function loadGrid (app, viewport, settings) {
  ...
  for (const hex of gr) {
    if (hex.tile === 'ShallowWater') {
      ...
      for (let hexToCheck of hexesInRange) {
  ...
  {texture = PIXI.utils.TextureCache['resources/img/tileset-borderless.png'];}
  ...
  for (let y = 0; y < settings.hexRows; y++) {
    for (let x = 0; x < settings.hexColums; x = x + 2) {
      ...
      let fantasyHexTile = new PIXI.Sprite(
        new PIXI.Texture(texture.baseTexture, texture.frame),
      );
      ...
      if (
       ...
       && lookup() <= 3) {
        ...
        let lairHexTile = new PIXI.Sprite(
          new PIXI.Texture(texture.baseTexture, texture.frame),
        );
  ...
  function drawRiver (hex) {
    ...
    for (let [i, hexToCheck] of hexesInRange.entries()) {
      ...
      // If there is a inferior archetype, choose as destination
        ...
        biomeTileset[hexToCheck.tile].z < biomeTileset[hexDestination.tile].z
      ) {
        hexDestination = hexToCheck;
      }
  ...
  function drawRiverTile (hex) {
    ...
    textureRivers.frame = new PIXI.Rectangle(
      tileCoords.x * 32,
      tileCoords.y * 48,
      32,
      48,
    );
    let riverSource = new PIXI.Sprite(
      new PIXI.Texture(textureRivers.baseTexture, textureRivers.frame),
    );
  ...
  for (const hex of gr) {
    ...
    let text = new PIXI.Text(coordinates.x + ',' + coordinates.y, {
      fontFamily: 'Arial',
      fontSize: fontSize,
      fill: 0x00_00_00,
      align: 'center',
...
function applySettings (app, viewport) {
  ...
  if ($('#setMapHash').is(':checked')) {
    ...
    settings.hexColums = Number.parseInt($('#hexColums').val()) || (width - 100) / 54;
    settings.hexRows = Number.parseInt($('#hexRows').val()) || (height - 100) / 72;
    settings.lineColor = 0x99_99_99;
    settings.hideCoords = $('#hideCoords').is(':checked');
    settings.hideGrid = $('#hideGrid').is(':checked');
    settings.contourInterval_0 =
      Number.parseFloat($('#contourInterval_0').val()) || 0.2;
    settings.contourInterval_1 =
      Number.parseFloat($('#contourInterval_1').val()) || 0.3;
    settings.contourInterval_2 =
      Number.parseFloat($('#contourInterval_2').val()) || 0.5;
    settings.contourInterval_3 =
      Number.parseFloat($('#contourInterval_3').val()) || 0.7;
    settings.contourInterval_4 =
      Number.parseFloat($('#contourInterval_4').val()) || 0.9;
    //Elevation Noise
    ...
    settings.frequencyElevation =
      Number.parseFloat($('#frequencyElevation').val()) || 0.8;
    settings.redistributionElevation =
      Number.parseFloat($('#redistributionElevation').val()) || 1;
    settings.elevationOctaves_0 =
      Number.parseFloat($('#elevationOctaves_0').val()) || 1;
    settings.elevationOctaves_1 =
      Number.parseFloat($('#elevationOctaves_1').val()) || 0.5;
    settings.elevationOctaves_2 =
      Number.parseFloat($('#elevationOctaves_2').val()) || 0.25;
    settings.elevationOctaves_3 =
      Number.parseFloat($('#elevationOctaves_3').val()) || 0.12;
    ...
    settings.frequencyMoisture =
      Number.parseFloat($('#frequencyMoisture').val()) || 0.8;
    settings.redistributionMoisture =
      Number.parseFloat($('#redistributionMoisture').val()) || 1;
    settings.moistureOctaves_0 = Number.parseFloat($('#moistureOctaves_0').val()) || 1;
    settings.moistureOctaves_1 =
      Number.parseFloat($('#moistureOctaves_1').val()) || 0.5;
    settings.moistureOctaves_2 =
      Number.parseFloat($('#moistureOctaves_2').val()) || 0.25;
    settings.moistureOctaves_3 =
      Number.parseFloat($('#moistureOctaves_3').val()) || 0.12;
  }
