...
const ClientGuest = require("./clientGuest");
...
class RPCClientBase {
  constructor(webSocket) {
    ...
    [...actionMehtodNameList].map((method) => {
      const middlewarePattern =
        "middleware" + method[0].toUpperCase() + method.slice(1);
      const m = middlewareMethodNameList.filter(
        (m) => middlewarePattern.indexOf(m) === 0
      );
      if (m.length)
        this.__actionsMiddlewareMap.set(
          method,
          m.map((m) => this[m])
        );
    });
    ...
    this.__webSocketEvents = {
      ...
      close: () => console.log("WebSocketClient connection closed."),
  ...
  __webSocketAddEvents() {
    Object.entries(this.__webSocketEvents).map(([k, f]) =>
      this.webSocket.on(k, f)
    );
  }
  __webSocketDelEvents() {
    Object.entries(this.__webSocketEvents).map(([k, f]) =>
      this.webSocket.off(k, f)
    );
  }
  ...
  async parseRPCMethod(rpcID, action, data) {
    ...
    if (middlewares) middlewares.map((f) => f(action, data, this, this.user));
  ...
  sendJSONResult(rpcID, result) {
    ...
    if (!(result instanceof Result)) {
      console.error(
        "WebSocketClient parse client message error: " + result?.message
      );
